	...
	if $attack_cooldown.is_stopped():
		if current_target_ref and current_target_ref.get_ref():
			...
			if $aggro_raycast.cast_to.length() < striking_distance:
				...
				var number_of_swords = 5
				for i in range(number_of_swords):
					var inst = small_red_sword_scene.instance()
					get_parent().add_child(inst)
					inst.global_position = target.global_position + Vector2(cos(i * TAU / number_of_swords), sin(i * TAU / number_of_swords)) * 30
					inst.rotation = (target.global_position - inst.global_position).angle()
...
func get_intended_direction():
	...
	if current_target_ref and current_target_ref.get_ref():
		var target = current_target_ref.get_ref()
		var there_to_here = global_position - target.global_position
		var target_position = target.global_position + there_to_here.normalized() * 300
